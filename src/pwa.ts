...
// 'activated' event and calls window.location.reload() automatically. No
// banner, no user prompt — appropriate here because the queue UI is
// read-only (display) or only persists transient input (patient number is
// in localStorage, the in-progress 4-digit input is trivially re-typable).
//
